...
<?php 
$periksa=$mysqli->query("select * from barang where jumlah <=3");
while($q=$periksa->fetch_array()){	
	if($q['jumlah']<=3){	
		?>	
		<script>
...
<?php 
$per_hal=10;
$jumlah_record=$mysqli->query("SELECT COUNT(*) from barang");
$jumlah_record->data_seek(0); 
$jum=$jumlah_record->fetch_array()[0];
...
<table class="table table-hover">
	...
	<?php 
	if(isset($_GET['cari'])){
		$cari=$mysqli->real_escape_string($_GET['cari']);
		$brg=$mysqli->query("select * from barang where nama like '$cari' or jenis like '$cari'");
	}else{
		$brg=$mysqli->query("select * from barang limit $start, $per_hal");
	}
	$no=1;
	...
	<tr>
		...
		<?php 
			...
			$x=$mysqli->query("select sum(modal) as total from barang");	
			$xx=$x->fetch_array();			
