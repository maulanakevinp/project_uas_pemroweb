...
<form action="" method="get">
	<div class="input-group col-md-5 col-md-offset-7">
		...
		<select type="submit" name="tanggal" class="form-control" onchange="this.form.submit()">
			...
			<?php 
			$pil=$mysqli->query("select distinct tanggal from barang_laku order by tanggal desc");
...
if(isset($_GET['tanggal'])){
	$tanggal=$mysqli->real_escape_string($_GET['tanggal']);
...
<table class="table">
	...
	if(isset($_GET['tanggal'])){
		$tanggal=$mysqli->real_escape_string($_GET['tanggal']);
		$brg=$mysqli->query("select * from barang_laku where tanggal like '$tanggal' order by tanggal desc");
	}else{
		$brg=$mysqli->query("select * from barang_laku order by tanggal desc");
	}
	$no=1;
	...
	<tr>
		...
		if(isset($_GET['tanggal'])){
			$tanggal=$mysqli->real_escape_string($_GET['tanggal']);
			$x=$mysqli->query("select sum(total_harga) as total from barang_laku where tanggal='$tanggal'");	
			$xx=$x->fetch_array();			
	...
	<tr>
		...
		if(isset($_GET['tanggal'])){
			$tanggal=$mysqli->real_escape_string($_GET['tanggal']);
			$x=$mysqli->query("select sum(laba) as total from barang_laku where tanggal='$tanggal'");	
			$xx=$x->fetch_array();			
			echo "<td><b> Rp.". number_format($xx['total']).",-</b></td>";
		}else{
